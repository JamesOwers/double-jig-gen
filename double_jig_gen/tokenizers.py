...
def abc_to_events(abc_str):
    ...
    for element in note_stream:
        if isinstance(element, music21.note.Note):
            events.append(
                dict(
                    start_quartertime=element.offset,
...
class ABCParser:
    ...
    def __init__(self, abc_str):
        ...
        self.abc_dict = parse_abc(abc_str)
        self.tune_str = self.abc_dict["tune_str"]
    ...
    def tokens(self):
        if self._tokens is None:
            self.abc_handler.tokenize(self.tune_str)
            self._tokens = [token.src for token in self.abc_handler.tokens]
        return self._tokens
