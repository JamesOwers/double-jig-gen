#!/usr/bin/env python
"""Script to download and create the splits for folkrnn data."""
import fire
import logging
...
def make_splits(
    ...
    nr_kept = int(np.rint(len(tune_lens)*percentile))
    max_tune_len = sorted(tune_lens)[nr_kept - 1]
    LOGGER.info(f"Choosing the shortest {percentile*100}% of tunes for train/valid/test")
